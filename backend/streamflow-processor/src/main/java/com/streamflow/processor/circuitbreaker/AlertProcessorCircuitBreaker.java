...
/**
 * Bridges the Resilience4j {@code alertProcessor} circuit breaker with Redis and Spring's event
 * bus.
 *
 ...
 *
 * <p>SPEC-12 R6 — a {@link CircuitBreakerStateEvent} is published on {@link
 * ApplicationEventPublisher} so SPEC-14 can broadcast the change to WebSocket clients.
 *
 * <p>Design note (SPEC-12 §4 / Open Question Q1): Resilience4j does not natively support a truly
 * distributed circuit breaker with consensus. The Redis write is a "last-writer-wins" view — useful
 * for dashboarding but not a distributed consensus. Each processor instance runs its own
 * Resilience4j state machine; they all write to the same Redis key, so the dashboard shows
 * whichever instance last transitioned. This trade-off is intentional and documented in the project
 * README.
 */
@Slf4j
@Component
...
public class AlertProcessorCircuitBreaker {
   ...
   *
   * <p>Project Plan §7 shows {@code cb_state:alert_processor:{streamId}} but because Resilience4j
   * uses a single CB per name (not per stream), SPEC-12 R5 specifies a single key {@code
   * cb_state:alert_processor}. See deviation note in spec.
   */
   ...
   *
   * <p>The listener is registered once (idempotent — Resilience4j deduplicates by reference
   * equality). It writes Redis and publishes a Spring event on every transition: CLOSED → OPEN,
  ...
  public void registerStateChangeListener() {
    ...
    cb.getEventPublisher()
        .onStateTransition(
            event -> {
              ...
              String reason =
                  "Circuit breaker transitioned from " + previousState + " to " + currentState;

              log.info(
                  "SPEC-12: CB '{}' state transition: {} → {} at {}ms",
                  CB_NAME,
                  previousState,
                  currentState,
                  now);
    ...
    log.info(
        "SPEC-12: AlertProcessorCircuitBreaker listener registered; initial state={}",
        cb.getState().name());
  }

  /**
   * Returns the current CB state as a string, reading from the local Resilience4j registry (not
   * Redis, to avoid a network round-trip on every snapshot cycle).
   *

 ...
 *
 * <p>SPEC-17 R4: persists {@link AlertEventDTO} records asynchronously with bounded
 * backpressure. The repository:
 * <ul>
 *   <li>Derives the {@code date_bucket} (daily, format {@code yyyy-MM-dd}) from the
 *       alert's {@code timestamp} field.</li>
...
public class CassandraAlertRepository {
     ...
     *
     * <p>SPEC-17 R4: derives {@code date_bucket} as a daily bucket from the alert's
     * {@code timestamp} (epoch-millis).
     *
    ...
    public void persist(AlertEventDTO alert) {
        ...
        try {
            ...
            log.warn("SPEC-17: interrupted waiting for alert Cassandra write permit stream={}", alert.streamId());
            return;
        }

        if (!acquired) {
            log.warn("SPEC-17: alert Cassandra write backpressure exceeded — dropping alert alertId={}",
                    alert.alertId());
            return;
        }
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                log.debug("SPEC-17: alert persisted: alertId={} stream={} type={}",
                        alert.alertId(), alert.streamId(), alert.alertType());
            } catch (Exception e) {
                log.error("SPEC-17: failed to persist alert alertId={} stream={}: {}",
    ...
    /**
     * Returns the formatted daily bucket for the given epoch-millis timestamp.

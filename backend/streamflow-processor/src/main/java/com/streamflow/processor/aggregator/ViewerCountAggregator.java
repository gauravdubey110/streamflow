 ...
 * <p>SPEC-04 design:
 * <ul>
 *   <li>Key schema: {@code viewer_count:{streamId}} (single sliding window per stream)</li>
 *   <li>Score      = event timestamp in ms (enables ZREMRANGEBYSCORE eviction)</li>
 *   <li>Member     = viewerId (string UUID — idempotent ZADD is naturally idempotent)</li>
 *   <li>TTL        = 600 s (10 min) — set on first ZADD; refreshed on every write</li>
 * </ul>
 *
 * <p>Thread-safety: Redis operations are inherently atomic per command.
 * The {@code knownStreamIds} set is a {@link ConcurrentHashMap} key-set.
 */
@Slf4j
@Component
public class ViewerCountAggregator {
    ...
    /**
     * Tracks all stream IDs seen so far — used by the eviction scheduler
     * ({@link com.streamflow.processor.scheduler.StaleViewerEvictionTask}) to know
     * which keys to clean up.
     */
     ...
     *
     * <p>SPEC-04 R2: {@code ZADD viewer_count:{streamId} <timestampMs> <viewerId>}.
     * If the key is new, also sets its TTL to avoid leaking idle-stream keys.
     *
     ...
     *
     * <p>SPEC-04 R3: called every 10 s by {@link com.streamflow.processor.scheduler.StaleViewerEvictionTask}.
     * Handles missing DROP events for viewers who disconnected without sending a DROP.
     *
     * @param streamId target stream
     */
    ...
    /**
     * Returns the set of all stream IDs for which this aggregator has seen events.
     * Used by the eviction scheduler to iterate over active streams.
     */

...
/**
 * Alert rule that fires when the rolling 30-second viewer delta drops below {@code -10%} of the
 * current mean viewer count.
 *
 * <p>SPEC-11 R2 — ViewerDropRule:
 *
 * <ul>
 *   <li>On each evaluation, pushes the current {@code viewerDelta} into the Redis Sorted Set {@code
 *       viewer_delta:{streamId}} scored by the current epoch-ms.
 *   <li>Prunes entries older than {@code streamflow.alerts.viewer-drop.window-seconds} (default 30
 *       s) via {@code ZREMRANGEBYSCORE}.
 *   <li>Computes the sum of all deltas in the window. If {@code sum < -(dropThresholdPct/100) *
 *       liveViewerCount}, fires WARNING.
 *   <li>Severity is always WARNING (spec does not specify CRITICAL for this rule).
 * </ul>
 *
 * <p>The sorted-set key has a TTL of {@code window + 30 s} refreshed on each write. This rule is
 * otherwise stateless.
 */
@Slf4j
@Component
public class ViewerDropRule implements AlertRule {
  ...
  public Optional<AlertEventDTO> evaluate(
    ...
    Set<ZSetOperations.TypedTuple<String>> entries =
        redisTemplate.opsForZSet().rangeByScoreWithScores(key, (double) cutoffMs, (double) nowMs);

    if (entries == null || entries.isEmpty()) {
      return Optional.empty();
    }

    long deltaSum = entries.stream().mapToLong(e -> parseLong(e.getValue())).sum();
    ...
    log.debug(
        "ViewerDropRule triggered: stream={} deltaSum={} threshold={} actualDropPct={}",
        streamId,
        deltaSum,
        threshold,
        actualDropPct);

 ...
 *
 * <p>SPEC-04 R3: runs every 10 seconds and removes members whose score
 * (join timestamp) is older than 5 minutes from every known stream's
 * {@code viewer_count:{streamId}} key.
 *
 * <p>This handles the case where a viewer disconnects without sending a
 * {@code DROP} event — without eviction the sorted set would grow unboundedly.
 *
 * <p>Scheduling is enabled at the application level via
 * {@link org.springframework.scheduling.annotation.EnableScheduling} on
 * {@link com.streamflow.processor.StreamProcessorApplication}.
 */
@Slf4j
@Component
public class StaleViewerEvictionTask {
     ...
     *
     * <p>Runs every 10 000 ms (SPEC-04 R3). Fixed-delay ensures the next run
     * starts 10 s after the previous one finishes, preventing overlap when

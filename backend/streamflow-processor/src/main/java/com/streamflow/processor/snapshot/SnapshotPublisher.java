 ...
 *
 * <ul>
 *   <li>R1 – Runs {@code @Scheduled(fixedRate = 1000)}; discovers active streams via {@code
 *       SMEMBERS active_streams} in Redis.
 *   <li>R2 – Builds a snapshot with {@code liveViewerCount} ({@code ZCARD}) and {@code
 *       viewerDelta}; all other numeric fields default to 0/empty.
 *   <li>R3 – Writes JSON to {@code stream_snapshot:{streamId}} with a 30-second TTL.
 *   <li>R4 – Publishes snapshot to the {@code metrics-aggregated} Kafka topic, keyed by {@code
 *       streamId}.
 *   <li>R5 – Logs a WARN if the scheduled run takes more than 800ms.
 * </ul>
 *
 * <p>SPEC-09 additions:
 *
 * <ul>
 *   <li>R4 – Reads {@code stream_health:{streamId}} hash written by {@link StreamHealthConsumer}
 *       and populates {@code healthScore} (via {@link HealthScoreCalculator}) and {@code
 *       p95LatencyMs} (= {@code encoderLatencyMs}) in the snapshot.
 *   <li>AC4 – If the health hash has expired (TTL exceeded 60 s), falls back to a default health
 *       score of 50.0 and logs a WARN.
 * </ul>
 *
 * <p>SPEC-10 additions:
 *
 * <ul>
 *   <li>R4 – Calls {@link QualityDistAggregator#getDistributionPct(String)} to populate {@code
 *       qualityDistribution} in the snapshot.
 *   <li>R4 – Calls {@link QualityDistAggregator#getBufferRatePct(String)} to populate {@code
 *       bufferRatePct} in the snapshot (replaces the placeholder 0.0).
 *   <li>The real {@code bufferRatePct} is also passed into {@link HealthScoreCalculator#compute} so
 *       the health score reflects actual buffering load (previously always 0.0 per the SPEC-09
 *       placeholder).
 * </ul>
 *
 * <p>SPEC-11 additions:
 *
 * <ul>
 *   <li>R5 – Calls {@link AlertEngine#getActiveAlertCount(String)} to populate {@code activeAlerts}
 *       in the snapshot (replaces the placeholder 0).
 * </ul>
 *
 * <p>SPEC-12 additions:
 *
 * <ul>
 *   <li>R7 – Reads {@code circuitBreakerState} from {@link
 *       AlertProcessorCircuitBreaker#getCurrentState()} (local Resilience4j registry) instead of
 *       the previous hard-coded {@code "CLOSED"} placeholder.
 * </ul>
 *
 * <p>SPEC-17 additions:
 *
 * <ul>
 *   <li>R5 – Calls {@link CassandraMetricSnapshotRepository#persist(StreamMetricSnapshotDTO)} after
 *       each successful snapshot. The repository gates writes to once per minute per stream, so
 *       calling it every second is safe — most calls are no-ops.
 * </ul>
 *
 * <p><b>Scaling note (NFR1):</b> the single-threaded {@code @Scheduled} task is sufficient for ≤ 10
 * streams. Beyond that, the task should be partitioned by stream range or replaced with a reactive
 * pipeline (e.g. Project Reactor with a bounded parallel scheduler) and the {@code active_streams}
 * SMEMBERS scan moved to a cursor-based SSCAN to avoid blocking Redis.
 */
@Slf4j
@Component
public class SnapshotPublisher {
  ...
  /**
   * Redis key for the active-stream set maintained by {@link
  ...
  /**
   * Health score returned when the {@code stream_health:{streamId}} hash has expired (SPEC-09 AC4).
   * Represents a degraded-but-not-zero state so the dashboard does not show a misleading 100.
   */
  ...
  /**
   * In-memory map of previous-second viewer counts per stream. Used to compute {@code viewerDelta =
   * current − previous} (SPEC-05 R2).
   */
  ...
  public SnapshotPublisher(
    ...
    this.snapshotTimer =
        Timer.builder("streamflow.snapshot.duration")
   ...
   *
   * <ol>
   *   <li>Reads live viewer count from Redis (ZCARD).
   *   <li>Computes delta vs. previous run.
   *   <li>Writes JSON snapshot to Redis with a 30 s TTL.
   *   <li>Publishes snapshot to {@code metrics-aggregated} Kafka topic.
   * </ol>
   *
  ...
  public void publishSnapshots() {
        ...
        () -> {
          ...
          for (String streamId : activeStreams) {
            try {
              ...
              log.error(
                  "Failed to publish snapshot for stream={}: {}", streamId, e.getMessage(), e);
            }
          }
        });
    ...
    if (elapsedMs > WARN_THRESHOLD_MS) {
      log.warn(
          "Snapshot publish cycle took {}ms — exceeds {}ms threshold (SPEC-05 R5)",
          elapsedMs,
          WARN_THRESHOLD_MS);
    }
  }
   ...
   *
   * <p>SPEC-09: reads the {@code stream_health:{streamId}} Redis Hash to populate {@code
   * healthScore} and {@code p95LatencyMs}. Falls back to {@link #DEFAULT_HEALTH_SCORE_ON_EXPIRY}
   * with a WARN log when the hash has expired (SPEC-09 AC4).
   *
  ...
  private void publishSnapshotForStream(String streamId) throws JsonProcessingException {
    ...
    // SPEC-12 R7: read real CB state from local Resilience4j registry (replaces "CLOSED"
    // placeholder)
    ...
    redisTemplate.opsForValue().set(SNAPSHOT_KEY_PREFIX + streamId, snapshotJson, SNAPSHOT_TTL);
    ...
    log.trace(
        "Snapshot published: stream={} count={} delta={} health={}",
        streamId,
        currentCount,
        delta,
        health.healthScore);
  ...
  /**
   * Reads the {@code stream_health:{streamId}} Redis Hash and returns the derived health fields.
   *
   * <p>If the hash is absent (expired after 60 s without a health event), returns a fallback with
   * {@link #DEFAULT_HEALTH_SCORE_ON_EXPIRY} and 0 latency, and logs a WARN (SPEC-09 AC4 / Q1
   * resolution).
   *
   * <p>SPEC-10 R4: {@code bufferRatePct} is now supplied by the caller (from {@link
   * QualityDistAggregator}) instead of being hardcoded to 0.0.
   *
  ...
  private HealthFields readHealthFields(String streamId, double bufferRatePct) {
    ...
    if (raw == null || raw.isEmpty()) {
      log.warn(
          "Health hash '{}' is absent (expired or never written). "
              + "Defaulting healthScore={} for stream={} (SPEC-09 AC4)",
          key,
          DEFAULT_HEALTH_SCORE_ON_EXPIRY,
          streamId);
    ...
    try {
      ...
      log.warn(
          "Failed to parse health hash for stream={}: {}. Defaulting to {}.",
          streamId,
          e.getMessage(),
          DEFAULT_HEALTH_SCORE_ON_EXPIRY);
      return new HealthFields(DEFAULT_HEALTH_SCORE_ON_EXPIRY, 0);
  ...
  /**
   * Lightweight value object carrying the two health-derived fields that feed into the snapshot
   * (SPEC-09 R4).
   *

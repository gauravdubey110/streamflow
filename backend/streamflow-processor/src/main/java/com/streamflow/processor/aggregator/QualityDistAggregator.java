 ...
 * <p>SPEC-10 R2 — writes:
 * <ul>
 *   <li>{@code HINCRBY quality_dist:{streamId}:{minuteBucket} <quality> 1}
 *       on JOIN and QUALITY_SWITCH events.</li>
 *   <li>{@code HINCRBY buffer_count:{streamId}:{minuteBucket} TOTAL 1}
 *       on every event.</li>
 *   <li>{@code HINCRBY buffer_count:{streamId}:{minuteBucket} BUFFER 1}
 *       on BUFFER_START events.</li>
 *   <li>TTL 120 s on both keys after every write.</li>
 * </ul>
 *
 * <p>SPEC-10 R3 — reads (current + previous minute buckets merged):
 * <ul>
 *   <li>{@code getDistributionPct(streamId)} — returns quality tier percentages summing to
 *       100 ±0.1; returns all-zero map when no data exists.</li>
 *   <li>{@code getBufferRatePct(streamId)} — returns {@code 100 * BUFFER / TOTAL};
 *       returns 0 when TOTAL = 0.</li>
 * </ul>
 *
 * <p>NFR1: reads use two HGETALL commands (one per bucket) via the same Redis connection —
 * p99 &lt; 5 ms for the arithmetic operation.
 */
@Slf4j
@Component
public class QualityDistAggregator {
     ...
     * <ul>
     *   <li>JOIN, QUALITY_SWITCH → increment the quality tier counter.</li>
     *   <li>BUFFER_START → increment the BUFFER counter.</li>
     *   <li>All events (JOIN, DROP, QUALITY_SWITCH, BUFFER_START, BUFFER_END, ERROR) →
     *       increment the TOTAL counter.</li>
     * </ul>
     *
    ...
    public void recordEvent(ViewerEventDTO event) {
        ...
        if (type == EventType.JOIN || type == EventType.QUALITY_SWITCH) {
            ...
            String qualityField = event.quality() != null
            ...
            log.trace("quality_dist incremented: stream={} bucket={} quality={}", streamId, bucket, qualityField);
        }
     ...
     *
     * <p>Merges the current and previous minute buckets to smooth minute-boundary effects
     * (SPEC-10 R3). The returned map always contains all five quality tiers (even if 0%),
     * and the values sum to 100 ±0.1 when there is data; returns all-zero map when
     * there is no data in either bucket.
     *
     ...
     *
     * <p>Merges the current and previous minute buckets (SPEC-10 R3).
     * Returns 0.0 when TOTAL = 0 (no events in either bucket).
     *
     * @param streamId the stream to query
    ...
    public double getBufferRatePct(String streamId) {
        ...
        log.trace("getBufferRatePct: stream={} buffer={} total={} rate={}", streamId, bufferCount, totalCount, rate);
        return rate;
    }
    ...
    /**
     * Returns the current minute bucket string, e.g. {@code "202406151430"}.
     * Format is {@code yyyyMMddHHmm} in UTC (SPEC-10 §4).
     */
    ...
    /**
     * Returns the previous minute bucket string (current − 1 minute).
     * Used by the read path to smooth minute-boundary effects (SPEC-10 R3).
     */
    ...
    private Map<String, Long> mergeQualityCounts(String streamId) {
        ...
        for (String bucket : new String[]{ currentBucket(), previousBucket() }) {
            Map<Object, Object> raw = redisTemplate.opsForHash()
                    .entries(qualityKey(streamId, bucket));
    ...
    private long[] mergeBufferCounts(String streamId) {
        ...
        for (String bucket : new String[]{ currentBucket(), previousBucket() }) {
            Map<Object, Object> raw = redisTemplate.opsForHash()
                    .entries(bufferKey(streamId, bucket));
    ...
    /**
     * Adjusts the largest quality-tier percentage so that all values sum exactly to 100.0
     * (corrects floating-point rounding error).
     */
    private void normalizeToHundred(Map<String, Double> pct) {
        ...
        String largestKey = pct.entrySet().stream()
    ...
    /**
     * Returns all quality-tier wire values as an ordered array.
     * Used in tests to verify complete coverage.
     */

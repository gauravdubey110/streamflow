 ...
 *
 * <ul>
 *   <li>Reads the {@code active_streams} Redis Set (same set maintained by {@link
 *       com.streamflow.processor.consumer.ViewerEventConsumer}) to discover streams.
 *   <li>Reads the current snapshot from {@code stream_snapshot:{streamId}} in Redis.
 *   <li>Runs each {@link AlertRule} on the snapshot.
 *   <li>Deduplicates: maintains a {@code last_alert:{streamId}:{alertType}} key in Redis (TTL =
 *       {@code streamflow.alerts.cooldown-seconds}, default 60 s). If the key is present, the alert
 *       is suppressed. If the rule clears, the key is deleted.
 *   <li>When an alert fires (and is not deduped), delegates to {@link AlertPublisher}.
 * </ul>
 *
 * <p>NFR1: with N=10 streams and 3 rules, one full evaluation cycle must complete in &lt;200 ms.
 * The implementation is single-threaded (shared {@code @Scheduled} executor) and performs only
 * Redis reads/writes (no Cassandra, no heavy computation) — well within the budget.
 *
 * <p>Shutdown: {@link #destroy()} calls {@link AlertPublisher#flush()} to drain in-flight Kafka
 * sends before the JVM exits (SPEC-11 §4).
 */
@Slf4j
@Component
public class AlertEngine {
   ...
   *
   * <p>A per-stream exception is caught so that one failing stream cannot abort the entire cycle
   * (defensive, same pattern as SnapshotPublisher).
   */
  ...
  public void evaluateAll() {
    ...
    if (elapsed > 200) {
      ...
      log.trace(
          "AlertEngine: evaluation cycle took {}ms for {} streams", elapsed, activeStreams.size());
    }
  }
   ...
   *
   * <p>SPEC-11 R5: used by {@link com.streamflow.processor.snapshot.SnapshotPublisher} to populate
   * {@code activeAlerts} in the snapshot.
   *
  ...
  /**
   * Called on Spring context shutdown. Flushes the alert Kafka producer so that in-flight sends
   * complete before the JVM exits (SPEC-11 §4).
   */
  @PreDestroy
  public void destroy() {
   ...
   *
   * <p>The snapshot is read from Redis as JSON. If the snapshot key is absent (stream active but
   * snapshot not yet written), evaluation is skipped silently.
   *
  ...
  private void evaluateStream(String streamId) {
    ...
    for (AlertRule rule : rules) {
      try {
        ...
        log.error(
            "AlertEngine: rule {} failed for stream={}: {}",
  ...
  private void handleResult(String streamId, Optional<AlertEventDTO> result, AlertType alertType) {
    ...
    log.info(
        "AlertEngine: alert published and dedup key set: key={} alertId={} severity={}",
        dedupKey,
        alert.alertId(),
        alert.severity());
  }
  ...
  private StreamMetricSnapshotDTO readSnapshot(String streamId) {
    ...
    try {
      ...
      log.warn(
          "AlertEngine: failed to parse snapshot JSON for stream={}: {}", streamId, e.getMessage());
      return null;
    }
  }
   ...
   *
   * <p>This mapping is used to build the dedup key without requiring rules to expose their type.
   * Adding a new rule requires adding a branch here.
   *
  ...
  private AlertType alertTypeFor(AlertRule rule) {
    ...
    log.warn(
        "AlertEngine: unknown rule type {}, using STREAM_DOWN as fallback dedup type",
        rule.getClass().getSimpleName());
  ...
  /**
   * Builds the dedup key map used by external components (e.g. SnapshotPublisher) to count active
   * alerts. Exposed for testing.
   */

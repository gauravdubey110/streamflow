 ...
 *
 * <p>Partition key: {@code (stream_id, date_bucket)} — hourly buckets keep
 * individual partitions bounded (Plan §8).
 * Clustering columns: {@code timestamp DESC, event_id ASC} for latest-first reads.
 *
 * <p>TTL is set at the {@code CREATE TABLE} level ({@code default_time_to_live = 604800})
 * so no per-row TTL statement is needed.
 */
@Data
@Builder
@NoArgsConstructor
...
public class ViewerEventEntity {
    ...
    /**
     * Partition column 2: hourly bucket derived from {@code timestamp}.
     * Format: {@code yyyy-MM-dd-HH} (e.g. {@code "2024-06-03-14"}).
     */

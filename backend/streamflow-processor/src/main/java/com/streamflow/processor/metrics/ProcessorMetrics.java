 ...
 * <ul>
 *   <li>{@code streamflow.events.consumed} (counter, tag {@code topic}) — incremented
 *       by {@link com.streamflow.processor.consumer.ViewerEventConsumer} and
 *       {@link com.streamflow.processor.consumer.StreamHealthConsumer}.</li>
 *   <li>{@code streamflow.alerts.fired} (counter, tags {@code severity,alertType}) — incremented
 *       by {@link com.streamflow.processor.alert.AlertEngine}.</li>
 *   <li>{@code streamflow.cb.state} (gauge, 0=CLOSED 1=HALF_OPEN 2=OPEN) — updated by
 *       {@link com.streamflow.processor.circuitbreaker.AlertProcessorCircuitBreaker}.</li>
 * </ul>
 *
 * <p>SPEC-20 NFR2 — Cardinality is bounded:
 * <ul>
 *   <li>{@code topic} tag: 2 values (viewer-events, stream-health).</li>
 *   <li>{@code severity} × {@code alertType}: 3 × 4 = 12 series maximum.</li>
 *   <li>{@code streamflow.cb.state}: 1 series.</li>
 * </ul>
 */
@Component
public class ProcessorMetrics {
    ...
    public ProcessorMetrics(MeterRegistry registry) {
        ...
        this.viewerEventsConsumed = Counter.builder(EVENTS_CONSUMED)
                .description("Number of events consumed from Kafka")
                .tag(TAG_TOPIC, "viewer-events")
                .register(registry);

        this.healthEventsConsumed = Counter.builder(EVENTS_CONSUMED)
                .description("Number of events consumed from Kafka")
    ...
    /**
     * Increments the {@code viewer-events} consumed counter.
     * Called by {@link com.streamflow.processor.consumer.ViewerEventConsumer}.
     */
    ...
    /**
     * Increments the {@code stream-health} consumed counter.
     * Called by {@link com.streamflow.processor.consumer.StreamHealthConsumer}.
     */
    ...
    /**
     * Increments the {@code streamflow.alerts.fired} counter for the given
     * severity and alert type (SPEC-20 R3).
     *
     * <p>A new counter series is created per (severity, alertType) pair the first
     * time this method is called with those tags. With 3 severities × 4 alert types
     * the maximum series count is 12, well within the NFR2 cardinality guard.
     *
     ...
     *
     * @param state the Resilience4j state string: {@code "CLOSED"}, {@code "HALF_OPEN"}, {@code "OPEN"}
     */

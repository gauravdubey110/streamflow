 ...
 *
 * <ul>
 *   <li>R1 – {@code @KafkaListener} on {@code viewer-events}, group {@code stream-processor-group},
 *       concurrency = 3 (set on the factory bean in {@link
 *       com.streamflow.processor.config.KafkaConsumerConfig}).
 *   <li>R2 – JOIN → {@code ZADD}; DROP → {@code ZREM}; other types ignored.
 *   <li>R5 – Idempotency is guaranteed by Redis Sorted-Set semantics: re-delivering the same
 *       viewerId ZADD only updates the score.
 *   <li>R6 – Manual ack after successful Redis write. On Redis failure the exception propagates and
 *       the error handler in {@link com.streamflow.processor.config.KafkaConsumerConfig} retries up
 *       to 3 times then dead-letters to {@code viewer-events.DLT}.
 * </ul>
 *
 * <p>SPEC-05 addition:
 *
 * <ul>
 *   <li>R1 – On first event for a stream, {@code SADD active_streams <streamId>} so that {@link
 *       com.streamflow.processor.snapshot.SnapshotPublisher} can discover active streams via {@code
 ...
 *
 * <ul>
 *   <li>R5 – Every viewer event is written asynchronously to the Cassandra {@code viewer_events}
 *       table via {@link CassandraViewerEventRepository}.
 * </ul>
 */
@Slf4j
@Component
public class ViewerEventConsumer {
   ...
   *
   * <p>The container factory is {@code viewerEventListenerContainerFactory} defined in {@link
   * com.streamflow.processor.config.KafkaConsumerConfig}, which sets concurrency = 3 and ack mode =
   * MANUAL_IMMEDIATE.
   *
   * @param event the deserialized {@link ViewerEventDTO}
   * @param acknowledgment manual acknowledgment handle; called after successful Redis write to
   *     commit the Kafka offset
   * @throws RuntimeException if the Redis write fails — the error handler will retry and eventually
  ...
  public void consume(@Payload ViewerEventDTO event, Acknowledgment acknowledgment) {
    try {
      ...
      log.error(
          "Failed to process viewer event for stream={} viewer={}: {}",
          event.streamId(),
          event.viewerId(),
          e.getMessage());
      throw e;
  ...
  /**
   * Registers {@code streamId} in the {@code active_streams} Redis Set and refreshes the set's TTL
   * (SPEC-05 R1).
   *
   * <p>{@code SADD} is idempotent — adding an already-present member is a no-op. The TTL is
   * refreshed on every call so active streams never expire while traffic keeps flowing.
   *

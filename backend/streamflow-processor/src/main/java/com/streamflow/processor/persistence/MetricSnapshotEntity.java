 ...
 *
 * <p>Partition key: {@code stream_id} (one partition per stream).
 * Clustering column: {@code minute_bucket DESC} for latest-first reads.
 *
 * <p>The {@code minute_bucket} is the {@code timestamp} value truncated to the
 * start of the current minute. This ensures exactly one row per stream per minute.
 *
...
public class MetricSnapshotEntity {
    ...
    /**
     * Clustering column: minute-truncated timestamp for this snapshot row.
     * Descending order for latest-first reads.
     */
    @PrimaryKeyColumn(name = "minute_bucket", type = PrimaryKeyType.CLUSTERED, ordinal = 0,

 ...
 *
 * <p>SPEC-17 R4: persists one {@link StreamMetricSnapshotDTO} row per stream per minute.
 * The repository:
 * <ul>
 *   <li>Truncates the snapshot's {@code snapshotTs} (epoch-millis) to the start of the
 *       current minute ({@code ChronoUnit.MINUTES}) — the {@code minute_bucket} column.</li>
 *   <li>Tracks the last persisted minute per stream in a {@link ConcurrentHashMap} so
 *       that the caller (SnapshotPublisher, running every second) only triggers a write
 *       once per minute per stream (SPEC-17 R5).</li>
 *   <li>Uses a {@link Semaphore} to cap concurrent in-flight writes (SPEC-17 R6).</li>
 * </ul>
 */
@Slf4j
@Repository
...
public class CassandraMetricSnapshotRepository {
    ...
    public CassandraMetricSnapshotRepository(CassandraOperations cassandraOperations) {
        ...
        this.writeExecutor = Executors.newFixedThreadPool(2,
                r -> {
    ...
    /**
     * Persists a metric snapshot to Cassandra if the current minute has not already
     * been written for this stream.
     *
     * <p>SPEC-17 R5: only one row per (stream, minute) is written. If the caller
     * invokes this method multiple times within the same minute (e.g. SnapshotPublisher
     * runs every second), all but the first call within a minute are no-ops.
     *
    ...
    public void persist(StreamMetricSnapshotDTO snapshot) {
        ...
        if (minuteBucket.equals(previous)) {
            log.trace("SPEC-17: snapshot for stream={} minute={} already written — skipping",
                    streamId, minuteBucket);
            return;
        }
        ...
        try {
            ...
            log.warn("SPEC-17: interrupted waiting for snapshot Cassandra write permit stream={}", streamId);
            return;
        }

        if (!acquired) {
            log.warn("SPEC-17: snapshot Cassandra write backpressure exceeded — dropping snapshot stream={}",
                    streamId);
            return;
        }
        ...
        CompletableFuture.runAsync(() -> {
            try {
                MetricSnapshotEntity entity = MetricSnapshotEntity.builder()
                        .streamId(streamId)
                ...
                log.debug("SPEC-17: metric snapshot persisted: stream={} minute={}",
                        streamId, minuteBucket);
            } catch (Exception e) {
                log.error("SPEC-17: failed to persist metric snapshot for stream={} minute={}: {}",
                        streamId, minuteBucket, e.getMessage(), e);
            } finally {
                semaphore.release();
            }
        }, writeExecutor);
    ...
    /**
     * Returns the last-written minute bucket for a stream, or {@code null} if never written.

 ...
 * <ul>
 *   <li>Reads the {@code stream_health:{streamId}} Redis Hash written by
 *       {@link StreamHealthConsumer} to get the latest {@code bitrateKbps}.</li>
 *   <li>Fires a WARNING alert when {@code bitrateKbps < streamflow.alerts.bitrate.min-kbps}
 *       (default 2500).</li>
 *   <li>If the health hash is absent (expired or never written), the rule clears
 *       silently — no alert is raised for missing data.</li>
 * </ul>
 *
 * <p>This rule is stateless: all data lives in the Redis health hash maintained
 * by {@link StreamHealthConsumer}.
 */
@Slf4j
@Component
public class BitrateDegradationRule implements AlertRule {
    ...
    public Optional<AlertEventDTO> evaluate(StreamMetricSnapshotDTO snapshot,
        ...
        try {
            ...
            log.warn("BitrateDegradationRule: cannot parse bitrateKbps='{}' for stream={}",
                    bitrateRaw, streamId);
            return Optional.empty();
        }
        ...
        log.debug("BitrateDegradationRule triggered: stream={} bitrateKbps={} threshold={}",
                streamId, bitrateKbps, minBitrateKbps);

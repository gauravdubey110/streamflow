 ...
 * <ul>
 *   <li>AC3 — Inject 200 BUFFER_START events into a single stream within 10s →
 *       snapshot's {@code bufferRatePct} rises above 5% within 30s.</li>
 *   <li>AC1 — After normal load, snapshot {@code qualityDistribution} sums to
 *       100 ±0.5 and all five tiers are present.</li>
 * </ul>
 *
 * <p>Uses a full {@code @SpringBootTest} context so the Kafka listener chain,
 * {@link QualityDistAggregator}, and {@link com.streamflow.processor.snapshot.SnapshotPublisher}
 * all participate.
 */
@Testcontainers
@SpringBootTest
...
class QualityDistAggregatorIT {
    ...
    /**
     * SPEC-10 AC3: inject BUFFER_START events into a single stream;
     * within 45s the snapshot's bufferRatePct should exceed 5%.
     *
     * <p>Strategy: interleave JOIN and BUFFER_START events so they are processed
     * by the Kafka consumer in proportion, rather than all JOINs first (which would
     * delay all BUFFER_STARTs past the 30s timeout since all events share one partition).
     * Ratio: 1 BUFFER_START per 4 JOINs = 20% buffer rate, well above the 5% threshold.
     * Total: 100 JOIN + 25 BUFFER_START = 125 events — small enough to process quickly.
     */
    @Test
        ...
        for (int i = 0; i < 25; i++) {
            ...
            for (int j = 0; j < 4; j++) {
                producer.send(new ProducerRecord<>(
                        ...
                        viewerEvent(streamId, "v-" + (i * 4 + j), EventType.JOIN,
                                VideoQuality.Q_1080P, null)
                ));
            }
            // 1 BUFFER_START event
            producer.send(new ProducerRecord<>(
                    KafkaTopics.VIEWER_EVENTS, streamId,
                    viewerEvent(streamId, "v-buf-" + i, EventType.BUFFER_START,
                            VideoQuality.Q_1080P, 1500L)
            ));
        }
        producer.flush();
        ...
        await().atMost(Duration.ofSeconds(45))
                ...
                .untilAsserted(() -> {
                    String json = redisTemplate.opsForValue()
                            .get("stream_snapshot:" + streamId);
                    assertThat(json)
                            .as("Snapshot should exist for stream=%s", streamId)
                            .isNotNull();
    ...
    /**
     * SPEC-10 AC1: after normal load (JOIN + QUALITY_SWITCH events with all 5 tiers),
     * the snapshot's qualityDistribution sums to 100 ±0.5 and contains all five tiers.
     */
    @Test
    ...
    private ViewerEventDTO viewerEvent(String streamId, String viewerId, EventType type,
                                       VideoQuality quality, Long bufferDurationMs) {

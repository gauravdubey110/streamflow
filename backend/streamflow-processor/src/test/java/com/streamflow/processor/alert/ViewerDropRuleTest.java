 ...
 *
 * <p>Because ViewerDropRule maintains rolling state in Redis (ZADD / ZREMRANGEBYSCORE),
 * a real Redis instance is required. Mocking a Redis sorted-set would be too complex
 * and would not provide meaningful coverage.
 */
@Testcontainers
class ViewerDropRuleTest {
    ...
    void accumulatedNegativeDeltas_exceedThreshold_returnsWarning() {
        ...
        Optional<AlertEventDTO> result = rule.evaluate(
                snapshot(streamId, 50_000L, -3_000L, 0.5), redisTemplate);
    ...
    private StreamMetricSnapshotDTO snapshot(String streamId, long liveCount, long delta,
                                             double bufferRate) {

 ...
 * <ul>
 *   <li>Stub the {@code alertKafkaTemplate} to throw a {@link RuntimeException} on every
 *       {@code send(...)} call, simulating a broker brownout.</li>
 *   <li>Call {@link AlertPublisher#publish(AlertEventDTO)} enough times to exceed the
 *       {@code minimumNumberOfCalls} threshold with a 100% failure rate → CB trips to OPEN.</li>
 *   <li>Verify the subsequent call is routed to the fallback (SPEC-12 R3/R4):
 *       the alert ID appears in {@code dropped_alerts:{streamId}} in Redis.</li>
 *   <li>Verify {@code cb_state:alert_processor} in Redis is set to {@code "OPEN"} (SPEC-12 R5).</li>
 * </ul>
 *
 * <p>The test uses a short CB config (sliding-window-size=10, min-calls=5) injected via
 * {@link DynamicPropertySource} to keep the test fast. Redis is a real Testcontainers
 * instance. Kafka is also Testcontainers (needed for context startup), but its template
 * is mocked.
 *
...
class AlertPublisherCircuitBreakerIT {
    ...
    static void overrideProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("resilience4j.circuitbreaker.instances.alertProcessor.sliding-window-size", () -> "10");
        registry.add("resilience4j.circuitbreaker.instances.alertProcessor.minimum-number-of-calls", () -> "5");
        registry.add("resilience4j.circuitbreaker.instances.alertProcessor.failure-rate-threshold", () -> "50");
        registry.add("resilience4j.circuitbreaker.instances.alertProcessor.wait-duration-in-open-state", () -> "30s");
    }

    /**
     * Replace the real alertKafkaTemplate with a mock that always fails.
     * This is the "broken Kafka" scenario for the circuit breaker test.
     *
     * <p>Note: we mock at the bean level because the CB annotation proxies the
     * {@link AlertPublisher#publish} method — the mock failure must be thrown
     * synchronously (or the CompletableFuture must complete exceptionally) to
     * be counted as a CB failure. We make the send() method throw synchronously.
     */
    ...
    /**
     * AC1 (adapted for IT): After enough failing calls, the CB transitions to OPEN
     * and subsequent calls are routed to the fallback which stores to dropped_alerts.
     *
     * <p>Also covers:
     * <ul>
     *   <li>AC3: while OPEN, dropped alerts accumulate in {@code dropped_alerts:{streamId}}.</li>
     *   <li>R5: {@code cb_state:alert_processor} = {@code "OPEN"} in Redis.</li>
     * </ul>
     */
    @Test
    void whenKafkaFails_cbOpens_fallbackStoresDroppedAlerts() {
        ...
        Awaitility.await()
                ...
                .untilAsserted(() -> {
                    String redisState = redisTemplate.opsForValue()
                            .get(AlertProcessorCircuitBreaker.CB_STATE_KEY);
        ...
        List<String> dropped = redisTemplate.opsForList()
                .range(AlertPublisher.DROPPED_KEY_PREFIX + streamId, 0, -1);
        assertThat(dropped)
     ...
     *
     * <p>This test verifies the cap by calling fallback directly (bypassing the CB proxy)
     * since waiting for 100+ real failures would be too slow for a unit/IT test.
     */
    @Test
    ...
    void fallback_droppedAlertsList_cappedAt100() {
        ...
        assertThat(listSize)
                .as("dropped_alerts list should be capped at %d (SPEC-12 R4)",

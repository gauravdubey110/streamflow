...
/**
 * End-to-end integration test: produce {@link ViewerEventDTO} events to Kafka, assert the {@link
 * ViewerEventConsumer} processes them and updates Redis via {@link ViewerCountAggregator}.
 *
 ...
 *
 * <ul>
 *   <li>{@code KafkaContainer} (Confluent 7.5.3) — matches dev docker-compose
 *   <li>{@code GenericContainer} with {@code redis:7.2-alpine}
 * </ul>
 *
 * <p>Cassandra auto-configuration is excluded via {@code spring.autoconfigure.exclude} because
 * SPEC-17 implements the Cassandra schema — there is no Cassandra in this spec.
 *
 * <p>Each test method uses a unique stream ID (UUID suffix) to prevent Redis key collisions across
 * tests sharing the same containers. Redis is flushed before each test to avoid cross-test
 * contamination.
 */
@Testcontainers
@SpringBootTest
...
class ViewerEventConsumerIT {
  ...
  @Autowired private ViewerCountAggregator viewerCountAggregator;

  @Autowired private RedisTemplate<String, String> redisTemplate;

  /**
   * Flush Redis before each test to prevent cross-test contamination. Each test also uses a unique
   * stream ID, so this is belt-and-suspenders.
   */
  @BeforeEach
  void flushRedis() {
  ...
  void ac2_thousandJoinsMinus200DropsYieldEightHundred() {
    ...
    for (int i = 0; i < 200; i++) {
      producer.send(
          new ProducerRecord<>(
              KafkaTopics.VIEWER_EVENTS, streamId, dropEvent(streamId, "viewer-" + i)));
    }
    producer.flush();
  ...
  void ac3_sameJoinFiveTimesIsIdempotent() {
    ...
    for (int i = 0; i < 5; i++) {
      producer.send(
          new ProducerRecord<>(KafkaTopics.VIEWER_EVENTS, streamId, joinEvent(streamId, viewerId)));
    }
    producer.flush();

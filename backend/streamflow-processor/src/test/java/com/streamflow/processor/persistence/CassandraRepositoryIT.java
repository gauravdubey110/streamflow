 ...
 * <p>This test class:
 * <ul>
 *   <li>Starts a real {@link CassandraContainer} (Cassandra 4.1) via Testcontainers.</li>
 *   <li>Uses {@link DynamicPropertySource} to override the contact-points so Spring Data
 *       Cassandra connects to the container port.</li>
 *   <li>Overrides {@code spring.autoconfigure.exclude} to re-enable Cassandra auto-config
 *       (which the test-profile {@code application.properties} disables by default).</li>
 *   <li>Applies the DDL from {@code infra/cassandra/init.cql} programmatically before
 *       the Spring context starts — because the one-shot init container is Docker-only.</li>
 * </ul>
 *
 * <p>Key assertions:
 * <ul>
 *   <li>AC1 – 1000 viewer events written; count query returns ≥ 1000 rows for the partition.</li>
 *   <li>AC2 – One metric snapshot per minute; only one row present after multiple persist() calls.</li>
 *   <li>AC3 – Alert written; row can be read back with matching fields.</li>
 * </ul>
 */
@Testcontainers
@SpringBootTest(
...
class CassandraRepositoryIT {
     ...
     *
     * <p>We use a raw {@link CqlSession} (not Spring's) because the Spring context
     * is not yet live when Testcontainers {@code @BeforeAll} runs.
     */
    @BeforeAll
    ...
    @Autowired
    private CassandraViewerEventRepository viewerEventRepository;

    @Autowired
    private CassandraMetricSnapshotRepository snapshotRepository;

    @Autowired
    private CassandraAlertRepository alertRepository;
    ...
    void ac1_thousandViewerEventsArePersistedAndQueryable() {
        ...
        for (int i = 0; i < 1000; i++) {
            ViewerEventDTO event = new ViewerEventDTO(
        ...
        await().atMost(Duration.ofSeconds(30))
                ...
                .untilAsserted(() -> {
                    var row = cqlSession.execute(
                            ...
                            "AND date_bucket = '" + dateBucket + "'").one();
    ...
    void ac1b_viewerEventsStoredLatestFirst() {
        ...
        viewerEventRepository.persist(new ViewerEventDTO(
                earlyId, streamId, "viewer-early",
                EventType.JOIN, VideoQuality.Q_720P, null, earlier, null));
        viewerEventRepository.persist(new ViewerEventDTO(
                lateId, streamId, "viewer-late",
                EventType.JOIN, VideoQuality.Q_1080P, null, later, null));

        await().atMost(Duration.ofSeconds(15))
                .pollInterval(Duration.ofMillis(250))
                .untilAsserted(() -> {
                    var rows = cqlSession.execute(
                            "SELECT event_id FROM streamflow.viewer_events " +
                            "WHERE stream_id = '" + streamId + "' " +
                            "AND date_bucket = '" + dateBucket + "' LIMIT 2").all();
    ...
    void ac2_metricSnapshotWrittenOncePerMinute() {
        ...
        Map<String, Double> qd = Map.of(
                "1080p", 45.0, "720p", 25.0, "480p", 20.0, "360p", 10.0);
        ...
        for (int i = 0; i < 5; i++) {
            StreamMetricSnapshotDTO snapshot = new StreamMetricSnapshotDTO(
                    streamId, "Test Stream", 50000L + i, 100L + i,
                    2.0, 40, qd, 95.0, "CLOSED", 0, baseTs + i * 1000L);
    ...
    void ac3_alertPersistedAndQueryableByDateBucket() {
        ...
        AlertEventDTO alert = new AlertEventDTO(
                alertId, streamId,
                AlertSeverity.CRITICAL, AlertType.HIGH_BUFFER_RATE,
                5.0, 12.3,
        ...
        await().atMost(Duration.ofSeconds(15))
                ...
                .untilAsserted(() -> {
                    var rows = cqlSession.execute(
                            ...
                            "WHERE stream_id = '" + streamId + "' " +
                            "AND date_bucket = '" + dateBucket + "'").all();

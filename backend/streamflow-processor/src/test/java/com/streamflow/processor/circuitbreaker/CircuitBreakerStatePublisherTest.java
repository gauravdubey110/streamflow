 ...
 *
 * <p>Verifies that a {@link CircuitBreakerStateEvent} is correctly converted to a
 * {@link CbStateEventDTO} and published to the {@code cb-events} Kafka topic.
 */
@ExtendWith(MockitoExtension.class)
class CircuitBreakerStatePublisherTest {

    @Mock
    private KafkaTemplate<String, CbStateEventDTO> cbKafkaTemplate;

    @InjectMocks
    private CircuitBreakerStatePublisher publisher;
    ...
    void onApplicationEvent_publishesToCbEventsTopic() {
        ...
        CircuitBreakerStateEvent event = new CircuitBreakerStateEvent(
                this, "CLOSED", "OPEN", "failure rate exceeded", ts);

 ...
 *
 * <p>SPEC-20 Test Plan — Integration: assert metrics increment after a known
 * number of events. Uses {@link SimpleMeterRegistry} so no Spring context is
 * needed (fast, in-process unit test).
 *
 * <p>Verifies all 6 custom metrics required by SPEC-20 R3:
 * <ul>
 *   <li>{@code streamflow.events.consumed} (counter, tag {@code topic})</li>
 *   <li>{@code streamflow.alerts.fired} (counter, tags {@code severity,alertType})</li>
 *   <li>{@code streamflow.cb.state} (gauge, 0=CLOSED 1=HALF_OPEN 2=OPEN)</li>
 *   <li>{@code streamflow.snapshot.duration} (timer — registered by SnapshotPublisher;
 *       verified via the registry rather than this component)</li>
 * </ul>
 */
class ProcessorMetricsTest {
    ...
    void incrementViewerEventsConsumed_incrementsCounter() {
        ...
        Counter counter = registry.find(ProcessorMetrics.EVENTS_CONSUMED)
        ...
        assertThat(counter.count())
                .as("Counter should be 3.0 after 3 increments")
                .isEqualTo(3.0);
    }
    ...
    void incrementAlertsFired_incrementsCounterWithCorrectTags() {
        ...
        Counter criticalHighBuffer = registry.find(ProcessorMetrics.ALERTS_FIRED)
        ...
        Counter warningViewerDrop = registry.find(ProcessorMetrics.ALERTS_FIRED)
    ...
    void updateCbState_closed_setsGaugeToZero() {
        ...
        assertThat(gauge.value())
                .as("CLOSED state should be gauge value 0")
                .isEqualTo(0.0);
    }
    ...
    void updateCbState_halfOpen_setsGaugeToOne() {
        ...
        assertThat(gauge).isNotNull();
        assertThat(gauge.value())
                .as("HALF_OPEN state should be gauge value 1")
                .isEqualTo(1.0);
    }
    ...
    void updateCbState_open_setsGaugeToTwo() {
        ...
        assertThat(gauge).isNotNull();
        assertThat(gauge.value())
                .as("OPEN state should be gauge value 2")
                .isEqualTo(2.0);
    }

 ...
 *
 * <ul>
 *   <li>Programmatically drive the CB through CLOSED → OPEN by recording failures against the
 *       Resilience4j registry.
 *   <li>Assert that the listener writes the new state to Redis (SPEC-12 R5).
 *   <li>Assert that a {@link CircuitBreakerStateEvent} is published on the Spring {@link
 *       ApplicationEventPublisher} (SPEC-12 R6).
 *   <li>Assert that {@link AlertProcessorCircuitBreaker#getCurrentState()} returns the current
 *       state string (SPEC-12 R7 — used by SnapshotPublisher).
 * </ul>
 *
...
class CircuitBreakerStateListenerTest {
  ...
  @Mock private ProcessorMetrics processorMetrics;
  ...
  void setUp() {
    ...
    cbComponent =
        new AlertProcessorCircuitBreaker(registry, redisTemplate, eventPublisher, processorMetrics);
  ...
  void stateTransitionToOpen_writesRedisKey() {
    ...
    assertThat(writtenValues).as("Redis should have been written with OPEN state").contains("OPEN");
  }
  ...
  /**
   * SPEC-12 R7: {@link AlertProcessorCircuitBreaker#getCurrentState()} returns the correct state
   * string for use by {@link com.streamflow.processor.snapshot.SnapshotPublisher}.
   */
  @Test
   ...
   *
   * <p>The test CB config: sliding window = 4, minimum calls = 2, failure rate = 50%. Recording 2
   * failures out of 2 calls (100%) exceeds the 50% threshold.
   */
  private static void forceOpen(CircuitBreaker cb) {
    // Record calls as failures to trip the CB
    for (int i = 0; i < 4; i++) {
      cb.onError(
          0, java.util.concurrent.TimeUnit.NANOSECONDS, new RuntimeException("test failure " + i));
    }
  }
}

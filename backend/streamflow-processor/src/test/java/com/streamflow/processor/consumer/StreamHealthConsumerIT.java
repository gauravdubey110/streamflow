 ...
 *
 * <p>SPEC-09 Test Plan — Integration: produce a degraded health event to the {@code stream-health}
 * topic and assert:
 *
 * <ol>
 *   <li>AC1 — {@code HGETALL stream_health:{streamId}} returns the correct field values.
 *   <li>Test-plan requirement — degraded event produces a computed score &lt; 60 (verified via
 *       {@link com.streamflow.processor.aggregator.HealthScoreCalculator} in the unit tests; here
 *       we verify the Redis hash fields are written correctly so the end-to-end chain would compute
 *       the expected score).
 * </ol>
 *
...
class StreamHealthConsumerIT {
  ...
  /**
   * Produce a normal health event and assert that all five hash fields appear in {@code
   * stream_health:{streamId}} within 15 seconds.
   *
   * <p>SPEC-09 AC1: {@code redis-cli HGETALL stream_health:stream-001} returns recent
   * bitrate/frame-drop/latency.
   */
  @Test
  void ac1_healthHashPopulatedInRedis() {
    String streamId = "it-health-ac1-" + UUID.randomUUID();
    StreamHealthEventDTO event =
  ...
  /**
   * SPEC-09 Test Plan: push a degraded health event and assert the hash fields that would cause a
   * {@link com.streamflow.processor.aggregator.HealthScoreCalculator} computation of score &lt; 60.
   *
   * <p>Degraded inputs: bitrate=2000 kbps, frameDrop=0.025, latency=300 ms. Expected score = 100 −
   * 40(buffer,0) − 25(frameDrop) − 15(latency) − 10(bitrate) = 10. (bufferRatePct is 0.0 in
   * SPEC-09; SPEC-10 provides it later.) Actual score via calculator = 100 − 0 − 25 − 15 − 10 = 50
   * → below 60.
   */
  @Test
  void degradedEvent_hashFieldsWrittenCorrectly() {
    String streamId = "it-health-degraded-" + UUID.randomUUID();
    StreamHealthEventDTO degraded =
        new StreamHealthEventDTO(
            UUID.randomUUID().toString(),
            streamId,
  ...
  /**
   * Sending two events for the same stream should result in the second event's values being present
   * in the hash (latest-write-wins semantics).
   */
  @Test
  void latestEvent_overwritesPreviousHashValues() {
    String streamId = "it-health-overwrite-" + UUID.randomUUID();

    StreamHealthEventDTO first =
        new StreamHealthEventDTO(
            UUID.randomUUID().toString(),
            streamId,
            4_500,

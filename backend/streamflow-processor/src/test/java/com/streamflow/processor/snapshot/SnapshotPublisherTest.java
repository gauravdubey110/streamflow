 ...
 *
 * <p>SPEC-05 Test Plan — Unit: verify that for each active stream the publisher writes a JSON
 * snapshot to Redis (SET with TTL) and sends a Kafka message to {@code metrics-aggregated}.
 *
 * <p>SPEC-12 additions: mock {@link AlertProcessorCircuitBreaker} and verify that the snapshot's
 * {@code circuitBreakerState} field reflects the CB state.
 *
 * <p>All external dependencies (Redis, Kafka, aggregator) are mocked so the test runs without any
 * infrastructure.
 */
...
class SnapshotPublisherTest {
  ...
  @Mock private KafkaTemplate<String, StreamMetricSnapshotDTO> snapshotKafkaTemplate;

  @Mock private ViewerCountAggregator viewerCountAggregator;

  @Mock private QualityDistAggregator qualityDistAggregator;

  @Mock private AlertEngine alertEngine;

  @Mock private AlertProcessorCircuitBreaker alertProcessorCircuitBreaker;
  ...
  void publishSnapshots_writesRedisSnapshotForEachActiveStream() {
    ...
    verify(valueOperations)
        .set(eq("stream_snapshot:" + streamId), anyString(), eq(Duration.ofSeconds(30)));
  }
  ...
  void publishSnapshots_sendsKafkaMessageForEachActiveStream() {
    ...
    verify(snapshotKafkaTemplate)
        .send(eq(KafkaTopics.METRICS_AGGREGATED), eq(streamId), any(StreamMetricSnapshotDTO.class));
  }
  ...
  void publishSnapshots_snapshotJsonContainsStreamIdAndCount() throws Exception {
    ...
    verify(valueOperations)
        .set(eq("stream_snapshot:" + streamId), jsonCaptor.capture(), eq(Duration.ofSeconds(30)));
  ...
  /**
   * When the {@code stream_health:{streamId}} hash is absent (empty map), the snapshot must contain
   * healthScore = 50.0 (SPEC-09 AC4 / Q1 resolution).
   */
  @Test
  @SuppressWarnings("unchecked")
  ...
  /**
   * SPEC-10 R4: when QualityDistAggregator returns a distribution, the snapshot must contain it.
   */
  @Test
  @SuppressWarnings("unchecked")
  void publishSnapshots_qualityDistribution_populatedInSnapshot() {
    ...
    Map<String, Double> dist =
        Map.of("1080p", 50.0, "720p", 25.0, "480p", 15.0, "360p", 7.0, "144p", 3.0);
  ...
  /**
   * When the {@code stream_health:{streamId}} hash is present with normal values, the snapshot's
   * healthScore must be computed (not 50.0 fallback). Normal: bitrate=4500, frameDrop=0.02,
   * latency=120 → score = 80.0.
   */
  @Test
  @SuppressWarnings("unchecked")
  ...
  /**
   * SPEC-12 R7: when the CB state is {@code OPEN}, the snapshot's {@code circuitBreakerState} field
   * must be {@code "OPEN"} (not the old hard-coded {@code "CLOSED"} placeholder).
   */
  @Test
  @SuppressWarnings("unchecked")

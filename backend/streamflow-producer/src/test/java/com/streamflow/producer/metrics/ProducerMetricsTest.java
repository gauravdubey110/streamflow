 ...
 *
 * <p>SPEC-20 Test Plan — verifies that the {@code streamflow.events.published} counter increments
 * correctly for both topic tags. Uses {@link SimpleMeterRegistry} so no Spring context is needed.
 */
class ProducerMetricsTest {
  ...
  void incrementViewerEventsPublished_incrementsViewerEventsCounter() {
    ...
    Counter counter =
        registry
            .find(ProducerMetrics.EVENTS_PUBLISHED)
    ...
    assertThat(counter)
        ...
        .isNotNull();
    assertThat(counter.count()).as("Counter should be 2.0 after 2 increments").isEqualTo(2.0);
  }

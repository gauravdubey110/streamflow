 ...
 *
 * <p>SPEC-13 Test Plan: verifies scenario modifiers (STREAM_DOWN → null,
 * VIEWER_DROP → high DROP rate, HIGH_BUFFER → high BUFFER_START rate, BITRATE_SPIKE → delegates).
 */
class ChaosAwareStrategyTest {
    ...
    void generate_bitrateSpike_delegatesToNormalStrategy() {
        when(mockInjector.activeScenario(STREAM_ID)).thenReturn(Optional.of(ChaosScenario.BITRATE_SPIKE));

 ...
 * </ul>
 *
 * <p>SPEC-09: adds a dedicated producer for {@link StreamHealthEventDTO} messages sent to the
 * {@code stream-health} topic every 2 seconds. Lower throughput than viewer events so no batching
 * tuning is applied.
 */
@Configuration
public class KafkaProducerConfig {
  ...
  /**
   * Producer factory with explicit tuning for high-throughput simulation. The {@code
   * spring.json.add.type.headers=false} setting ensures consumers without this library on the
   * classpath can still deserialise messages using only the schema defined in {@code
   * streamflow-common}.
   */
  @Bean
  ...
  /**
   * {@link KafkaTemplate} used by {@link com.streamflow.producer.simulator.ViewerEventProducer} to
   * send viewer events keyed by {@code streamId}.
   */
  @Bean
   ...
   *
   * <p>Uses the same bootstrap servers and serialization settings as the viewer-event factory but
   * omits high-throughput tuning (linger, batch size, compression) because health events are
   * emitted at only 1 msg/stream/2 s.
   */
  @Bean
   ...
   *
   * <p>Named {@code healthEventKafkaTemplate} to avoid ambiguity with {@link

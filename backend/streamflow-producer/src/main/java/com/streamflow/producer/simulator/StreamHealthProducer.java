 ...
 *
 * <ul>
 *   <li>bitrate: 4 500 kbps
 *   <li>frame-drop rate: 0.02
 *   <li>encoder latency: 100–150 ms (midpoint 125 ms)
 * </ul>
 *
 * <p>CDN edge nodes cycle through a small fixed set for realism. Production would source these from
 * a service-discovery registry; the fixed list is sufficient for simulation (SPEC-09 scope).
 */
@Slf4j
@Component
...
public class StreamHealthProducer {
  ...
  private static final String[] CDN_EDGE_NODES = {
    "edge-mumbai-01", "edge-delhi-01", "edge-california-01", "edge-london-01", "edge-singapore-01"
  };
   ...
   *
   * <p>Each invocation emits one event per configured stream, keyed by {@code streamId}, so all
   * events for a stream land in the same Kafka partition.
   */
    ...
    for (StreamDefinition stream : streams) {
      try {
        ...
        if (active.isPresent() && active.get() == ChaosScenario.STREAM_DOWN) {
          log.trace(
              "Skipping health event for stream={} (STREAM_DOWN chaos active)", stream.getId());
          continue;
        }
        StreamHealthEventDTO event =
            active.isPresent() && active.get() == ChaosScenario.BITRATE_SPIKE
                ? buildDegradedEvent(stream.getId())
                : buildEvent(stream.getId());
        healthEventKafkaTemplate
            .send(KafkaTopics.STREAM_HEALTH, stream.getId(), event)
            .whenComplete(
                (result, ex) -> {
                  if (ex != null) {
                    log.warn(
                        "Failed to send health event for stream={}: {}",
                        stream.getId(),
                        ex.getMessage());
                  } else {
                    log.trace(
                        "Health event sent: stream={} bitrate={} frameDrop={} latency={}",
                        stream.getId(),
                        event.bitrateKbps(),
                        event.frameDropRate(),
  ...
  /**
   * Returns a random value uniformly distributed within ±{@link #JITTER_FRACTION} of {@code
   * baseValue}.
   *
  ...
  /**
   * Returns a random double uniformly distributed within ±{@link #JITTER_FRACTION} of {@code
   * baseValue}.
   *
   * @param baseValue the nominal value
   * @return {@code baseValue * (1 ± JITTER_FRACTION)}

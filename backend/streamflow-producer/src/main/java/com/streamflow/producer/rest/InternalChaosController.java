 ...
 *
 * <p>SPEC-13 R3: exposed on port 8081 (the producer's {@code server.port}). Not intended for direct
 * client access — treated as an internal service-to-service API.
 *
 * <h3>Endpoints</h3>
 *
 * <ul>
 *   <li>{@code POST /internal/chaos} — starts a chaos session; returns {@code 202 Accepted} with
 *       {@code {"chaosId": "...", "startsAt": epochMs}}.
 *   <li>{@code DELETE /internal/chaos/{chaosId}} — cancels chaos; returns {@code 204 No Content} or
...
public class InternalChaosController {
   ...
   */
  @PostMapping
  public ResponseEntity<ChaosStartResponse> startChaos(
      @Valid @RequestBody ChaosStartRequest request) {
    log.info(
        "InternalChaosController.startChaos: stream={} scenario={} duration={}s",
        request.streamId(),
        request.scenario(),
        request.durationSeconds());

    String chaosId =
        chaosInjector.start(request.scenario(), request.streamId(), request.durationSeconds());
   ...
   *
   * <ul>
   *   <li>{@code scenario} — must be a valid {@link ChaosScenario} name.
   *   <li>{@code durationSeconds} — 1–300.
   *   <li>{@code streamId} — must not be blank.
   * </ul>
   */
  public record ChaosStartRequest(
      @NotBlank(message = "streamId must not be blank") String streamId,
      @NotNull(message = "scenario must not be null") ChaosScenario scenario,
   ...
   */
  public record ChaosStartResponse(String chaosId, long startsAt) {}
}

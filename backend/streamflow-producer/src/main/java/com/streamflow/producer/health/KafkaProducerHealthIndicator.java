 ...
 *
 * <p>SPEC-03 R7: reports {@code UP} only when at least one event has been published
 * in the last 10 seconds across any configured stream.
 *
 * <p>When simulation is disabled ({@link StreamSimulator#getProducers()} is empty),
 * the indicator returns {@code UP} with a note — this is intentional; a disabled
 * simulation is not an error state.
 */
@Component
...
public class KafkaProducerHealthIndicator implements HealthIndicator {
    ...
    public Health health() {
        ...
        if (producers.isEmpty()) {
            return Health.up()
                    .withDetail("note", "Simulation disabled or no streams configured")
                    .build();
        }
        ...
        if (anyHealthy) {
            ...
            return Health.down()
                    .withDetail("reason",
                            "No events published in the last 10 seconds on any stream")
                    .withDetail("totalStreams", producers.size())
                    .build();
        }
    }

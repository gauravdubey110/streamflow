 ...
 *
 * <p>SPEC-03 R3: starts one producer per configured stream on application startup. The aggregate
 * TPS is distributed evenly across streams ({@code perStreamTps = totalTps / numStreams}).
 *
...
public class StreamSimulator {
  ...
  /**
   * Start all per-stream producers on application startup. No-ops if {@code
   * streamflow.simulation.enabled=false}.
   */
  @PostConstruct
  public void start() {
    if (!simulationConfig.isEnabled()) {
      log.info(
          "Simulation disabled (streamflow.simulation.enabled=false). No events will be produced.");
      return;
    }
    ...
    if (streams.isEmpty()) {
      log.warn(
          "No streams configured under streamflow.simulation.streams[]. No events will be produced.");
      return;
    }

    int perStreamTps = Math.max(1, simulationConfig.getTps() / streams.size());
    log.info(
        "Starting StreamSimulator: {} streams, {}tps total, {}tps per stream",
        streams.size(),
        simulationConfig.getTps(),
        perStreamTps);

    for (StreamDefinition def : streams) {
      ViewerEventProducer producer =
          new ViewerEventProducer(
              def.getId(),
              def.getName(),
              perStreamTps,
   ...
   *
   * <p>SPEC-03 R6: each {@link ViewerEventProducer#stop()} waits up to 5s; called sequentially so
   * the overall budget is {@code 5s * numStreams}.
   */
  @PreDestroy
  public void stop() {
  ...
  /** Returns an unmodifiable view of all active producers. Used by the health indicator. */

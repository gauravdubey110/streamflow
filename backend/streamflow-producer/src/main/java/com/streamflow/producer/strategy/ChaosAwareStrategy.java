...
/**
 * Decorator around {@link NormalLoadStrategy} that applies chaos modifiers when a chaos session is
 * active for the target stream.
 *
 * <p>SPEC-13 R2 / Design Notes: this is the {@link Primary} {@link EventGenerationStrategy} bean so
 * Spring injects it into {@link com.streamflow.producer.simulator.StreamSimulator} and {@link
 * com.streamflow.producer.simulator.ViewerEventProducer}.
 *
 * <h3>Scenario behaviour</h3>
 *
 * <ul>
 *   <li>{@link ChaosScenario#VIEWER_DROP} — doubles the probability of a DROP event (from ~25 % to
 *       ~50 %). The generated event type is re-rolled with the chaos distribution.
 *   <li>{@link ChaosScenario#HIGH_BUFFER} — emits {@code BUFFER_START} at 12 % probability (vs. ~5
 *       % baseline) with a 500–3 000 ms buffer duration.
 *   <li>{@link ChaosScenario#BITRATE_SPIKE} — delegates to the normal strategy; the health event
 *       modulation is handled by {@link com.streamflow.producer.simulator.StreamHealthProducer}.
 *   <li>{@link ChaosScenario#STREAM_DOWN} — returns {@code null}; the caller ({@link
 *       com.streamflow.producer.simulator.ViewerEventProducer}) must skip publishing when {@code
 *       null} is returned.
 * </ul>
 */
@Slf4j
...
public class ChaosAwareStrategy implements EventGenerationStrategy {
   ...
   * @return a {@link ViewerEventDTO}, or {@code null} when {@link ChaosScenario#STREAM_DOWN} is
   *     active (callers must skip publishing nulls)
   */
  @Override
  ...
  private ViewerEventDTO generateWithHighBuffer(String streamId, String streamName) {
    ...
    // 0-11 → BUFFER_START (12%), 12-87 → JOIN (~76% of remainder ≈ 66.9%), 88-95 → DROP, 96-99 →
    // QUALITY_SWITCH
    EventType eventType;
    Long bufferDurationMs = null;

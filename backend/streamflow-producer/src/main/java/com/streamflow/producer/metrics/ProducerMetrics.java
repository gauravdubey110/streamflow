 ...
 * <ul>
 *   <li>tag {@code topic=viewer-events} — incremented by {@link com.streamflow.producer.simulator.ViewerEventProducer}</li>
 *   <li>tag {@code topic=stream-health} — incremented by {@link com.streamflow.producer.simulator.StreamHealthProducer}</li>
 * </ul>
 *
 * <p>SPEC-20 NFR2 — Cardinality is bounded: the {@code topic} tag has a fixed set of values
 * (2 topics) so series cardinality is well below the 5 000-series limit.
 */
@Component
public class ProducerMetrics {
    ...
    public ProducerMetrics(MeterRegistry registry) {
        this.viewerEventsPublished = Counter.builder(EVENTS_PUBLISHED)
                .description("Number of events successfully published to Kafka")
                .tag(TAG_TOPIC, "viewer-events")
                .register(registry);

        this.healthEventsPublished = Counter.builder(EVENTS_PUBLISHED)
                .description("Number of events successfully published to Kafka")
    ...
    /**
     * Increments the {@code viewer-events} published counter.
     * Called from {@link com.streamflow.producer.simulator.ViewerEventProducer}
     * on each successful Kafka send (SPEC-20 R3).
     */
    ...
    /**
     * Increments the {@code stream-health} published counter.
     * Called from {@link com.streamflow.producer.simulator.StreamHealthProducer}
     * on each successful Kafka send (SPEC-20 R3).
     */

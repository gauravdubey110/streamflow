 ...
 *
 * <ul>
 *   <li>{@code viewer-events} — 6 partitions, 2-hour retention
 *   <li>{@code stream-health} — 3 partitions, 2-hour retention
 *   <li>{@code alerts} — 3 partitions, 24-hour retention
 *   <li>{@code metrics-aggregated} — 3 partitions, 1-hour retention
 *   <li>{@code cb-events} — 3 partitions; circuit breaker state transitions (SPEC-14 R2/R3)
 * </ul>
 *
...
public final class KafkaTopics {
  ...
  /**
   * Internal topic: aggregated metric snapshots from the stream processor consumed by the API
   * gateway for WebSocket push.
   */
  public static final String METRICS_AGGREGATED = "metrics-aggregated";

  /**
   * Circuit-breaker state-transition events published by the stream processor and consumed by the
   * API gateway for WebSocket push (SPEC-14 R2/R3).
   *

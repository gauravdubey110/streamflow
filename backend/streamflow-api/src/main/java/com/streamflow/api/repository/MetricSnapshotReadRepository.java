 ...
 *
 * <p>SPEC-18 R4/R6: fetches metric snapshots for a given stream within a time range.
 * The {@code metric_snapshots} table partitions by {@code stream_id} only, so a single
 * partition scan with a clustering-key range filter covers the full request efficiently.
 *
 * <p>Uses {@link CqlOperations#query(String, RowMapper, Object...)} for typed result mapping.
 * The {@link ConditionalOnBean} guard ensures this bean is skipped when Cassandra
 * auto-configuration is excluded in tests that do not need Cassandra.
 */
@Slf4j
@Repository
...
public class MetricSnapshotReadRepository {
    ...
    public List<MetricSnapshotEntity> findByStreamAndRange(String streamId, long from, long to) {
        ...
        String cql = "SELECT stream_id, minute_bucket, live_viewer_count, buffer_rate_pct, " +
        ...
        RowMapper<MetricSnapshotEntity> mapper = (row, rowNum) ->
                MetricSnapshotEntity.builder()
                        .streamId(row.getString("stream_id"))
                        .minuteBucket(row.getInstant("minute_bucket"))
                        .liveViewerCount(row.isNull("live_viewer_count") ? null : row.getLong("live_viewer_count"))
                        .bufferRatePct(row.isNull("buffer_rate_pct") ? null : row.getDouble("buffer_rate_pct"))
                        ...
                        .quality1080pPct(row.isNull("quality_1080p_pct") ? null : row.getDouble("quality_1080p_pct"))
                        .quality720pPct(row.isNull("quality_720p_pct") ? null : row.getDouble("quality_720p_pct"))
                        .quality480pPct(row.isNull("quality_480p_pct") ? null : row.getDouble("quality_480p_pct"))
                        .quality360pPct(row.isNull("quality_360p_pct") ? null : row.getDouble("quality_360p_pct"))
                        .build();

        try {
            List<MetricSnapshotEntity> results = cqlOps.query(cql, mapper, streamId, fromInstant, toInstant);
            log.debug("SPEC-18: metric snapshots fetched: stream={} from={} to={} count={}",
                    streamId, fromInstant, toInstant, results.size());
            return results;
        } catch (Exception e) {
            log.error("SPEC-18: failed to query metric_snapshots for stream={}: {}", streamId, e.getMessage(), e);
            return List.of();
        }
    }
}

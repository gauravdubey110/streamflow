 ...
 *
 * <p>SPEC-13 R5: uses {@link RestTemplate} (not Feign) with a configurable base URL
 * ({@code streamflow.producer.base-url}) so it can be pointed at any producer instance
 * (local or Docker).
 *
 * <p>Throws {@link IllegalStateException} if the producer is unreachable; callers should
 * map this to an appropriate HTTP 502/503 response.
 */
@Slf4j
@Component
public class ProducerChaosClient {
    ...
    public ChaosResponse startChaos(String streamId, ChaosScenarioDTO scenario, int durationSeconds) {
        ...
        log.info("Forwarding chaos start to producer: url={} streamId={} scenario={} duration={}s",
                url, streamId, scenario, durationSeconds);

        try {
            ResponseEntity<ChaosResponse> response = restTemplate.postForEntity(
                    url, body, ChaosResponse.class);
            ChaosResponse result = response.getBody();
            log.info("Producer returned chaosId={}", result != null ? result.chaosId() : "null");
            return result;
        } catch (HttpClientErrorException e) {
            log.error("Producer returned error for chaos start: status={} body={}",

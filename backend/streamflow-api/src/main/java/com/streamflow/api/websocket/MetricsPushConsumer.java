 ...
 *
 * <p>SPEC-06 R2: Consumes {@link StreamMetricSnapshotDTO} from the
 * {@code metrics-aggregated} topic (group {@code api-gateway-group}) and
 * broadcasts each snapshot to the STOMP destination
 * {@code /topic/streams/{streamId}/metrics}.
 *
 * <p>Clients subscribe to {@code /topic/streams/{streamId}/metrics} and receive
 * a new message approximately every second (driven by {@code SnapshotPublisher}
 * in the processor module — SPEC-05).
 *
 * <p>Thread-safety: {@link SimpMessagingTemplate} is thread-safe; the Kafka
 * listener container dispatches on its own thread pool. Increasing concurrency
 * on the container factory ({@code snapshotListenerContainerFactory}) is safe
 * here as a result.
 */
@Slf4j
@Component
...
public class MetricsPushConsumer {
     ...
     */
    @KafkaListener(
            topics = "${streamflow.kafka.topics.metrics-aggregated:" + KafkaTopics.METRICS_AGGREGATED + "}",

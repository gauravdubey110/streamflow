 ...
 *
 * <ul>
 *   <li>{@code active_streams} — Set of active stream IDs (written by the processor).
 *   <li>{@code stream_snapshot:{streamId}} — JSON-encoded {@link StreamMetricSnapshotDTO} with a
 *       30-second TTL (written by {@code SnapshotPublisher} in the processor).
 * </ul>
 */
@Slf4j
@Service
...
public class StreamService {
   ...
   *
   * <p>SPEC-06 R3: {@code SMEMBERS active_streams} to get stream IDs, then GET each {@code
   * stream_snapshot:{id}} JSON and project it.
   *
   ...
   *
   * <p>SPEC-06 R4: Throws {@link StreamNotFoundException} (→ HTTP 404) if the Redis key is absent
   * (the snapshot TTL is 30 s, so an expired key means the stream is no longer active).
   *
   ...
   */
  private StreamMetricSnapshotDTO readSnapshot(String streamId) throws JsonProcessingException {
   ...
   *
   * <p>{@code streamName} is stubbed to {@code streamId} until stream-metadata persistence is added
   * in a later spec (see SPEC-06 §4).
   */

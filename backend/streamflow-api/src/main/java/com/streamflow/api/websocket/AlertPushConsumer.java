 ...
 *
 * <p>Consumes {@link AlertEventDTO} from the {@code alerts} topic (group
 * {@code api-gateway-group}) and broadcasts each alert to the STOMP destination
 * {@code /topic/streams/{streamId}/alerts}.
 *
 * <p>The outbound message is wrapped in an {@link AlertWsMessage} envelope that
 * adds the {@code type} discriminator field ({@code "ALERT_FIRED"}) so that the
 * React client can handle multiple message types on the same subscription
 * (SPEC-14 R4).
 *
 * <p>Thread-safety: {@link SimpMessagingTemplate} is thread-safe; Kafka listener
 * container dispatches on its own thread pool.
 */
@Slf4j
@Component
...
public class AlertPushConsumer {
     ...
     * to the STOMP destination for the alert's stream.
     *
    ...
    public void consume(@Payload AlertEventDTO alert) {
        ...
        AlertWsMessage message = new AlertWsMessage(
                AlertWsMessage.TYPE,
                alert.alertId(),
                alert.streamId(),
                alert.severity(),

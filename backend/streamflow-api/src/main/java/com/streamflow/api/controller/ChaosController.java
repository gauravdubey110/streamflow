 ...
 *
 * <p>SPEC-13 R4: forwards requests to the producer's internal API via
 * {@link ProducerChaosClient}.
 *
 * <h3>Endpoints</h3>
 * <ul>
 *   <li>{@code POST /api/v1/streams/{streamId}/chaos} — starts a chaos session;
 *       returns {@code 202 Accepted} with a {@link ChaosResponse}.</li>
 *   <li>{@code DELETE /api/v1/streams/{streamId}/chaos/{chaosId}} — cancels an
 *       active chaos session; returns {@code 204 No Content}.</li>
 * </ul>
 */
@Slf4j
@RestController
...
@Tag(name = "Chaos", description = "Chaos injection endpoints — trigger and cancel degradation scenarios")
public class ChaosController {
    ...
    @Operation(
            ...
            description = "Starts a degradation scenario on the specified stream. " +
    ...
    public ResponseEntity<ChaosResponse> startChaos(
        ...
        log.info("ChaosController.startChaos: stream={} scenario={} duration={}s",
                streamId, request.scenario(), request.durationSeconds());

        ChaosResponse response = producerChaosClient.startChaos(
                streamId, request.scenario(), request.durationSeconds());

 ...
 *       (default 24 hours). Throws {@link HistoryRangeException} (→ HTTP 400) if exceeded.</li>
 *   <li>For metric snapshots: issues a single partition range query (the {@code metric_snapshots}
 *       table partitions by {@code stream_id} only, so no bucket enumeration is needed).</li>
 *   <li>For alerts: enumerates daily buckets spanning the range and merges results in memory.</li>
 *   <li>Downsamples to HOUR granularity (keeps every 60th row, ascending) when requested.</li>
 *   <li>Caps results at {@value #MAX_POINTS} data points (SPEC-18 NFR2).</li>
 * </ul>
 *
 * <p>SPEC-18 Design Notes: alert severity filtering is applied in-memory (post-fetch)
 * rather than using ALLOW FILTERING in Cassandra.
 *
 * <p>The {@link ConditionalOnBean} annotation ensures this service is skipped when
 * Cassandra is not configured (e.g. in unit tests for other parts of the api module).
 */
@Slf4j
@Service
...
public class HistoryService {
     ...
     *
     * <p>SPEC-18 R1: results are ordered ascending by {@code snapshotTs} (oldest first).
     * SPEC-18 R4: HOUR granularity keeps every 60th row (one per hour) from the ascending list.
     *
     ...
     */
    public List<StreamMetricSnapshotDTO> getHistory(String streamId, long from, long to,
                                                     String granularity) {
        ...
        log.debug("SPEC-18: history query: stream={} from={} to={} granularity={} resultCount={}",
                streamId, from, to, granularity, rows.size());
     ...
     *
     * <p>SPEC-18 R2: results are ordered descending by {@code timestamp} (newest first).
     * Severity filter is applied in-memory.
     *
     * @param streamId        the stream to query
     * @param from            start of range (inclusive), epoch-milliseconds
     * @param to              end of range (inclusive), epoch-milliseconds
     ...
     * @throws HistoryRangeException if {@code to - from} exceeds the configured max range
     */
    public List<AlertEventDTO> getAlerts(String streamId, long from, long to,
        ...
        if (severityFilter != null && !severityFilter.isBlank()) {
            rows = rows.stream()
                    .filter(a -> severityFilter.equalsIgnoreCase(a.getSeverity()))
                    .toList();
        }
        ...
        log.debug("SPEC-18: alerts query: stream={} from={} to={} severity={} resultCount={}",
                streamId, from, to, severityFilter, sorted.size());
     ...
     *
     * <p>SPEC-18 R4: "every 60 rows → 1" — keeps indices 0, 60, 120, … from the
     * ascending-sorted input. This approximates one row per hour.
     *
    ...
    private StreamMetricSnapshotDTO toSnapshotDTO(MetricSnapshotEntity e) {
        ...
        return new StreamMetricSnapshotDTO(
                ...
                e.getMinuteBucket() != null
                        ? e.getMinuteBucket().toEpochMilli() : 0L
        );
    }

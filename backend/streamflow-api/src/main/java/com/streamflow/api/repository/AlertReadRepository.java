 ...
 *
 * <p>SPEC-18 R2/R6: fetches alerts for a given stream and time range, spanning one or more {@code
 * (stream_id, date_bucket)} partitions as needed.
 *
 * <p>Because the alerts table partitions by {@code (stream_id, date_bucket)}, the repository must
 * issue one query per daily bucket that intersects the requested range. Results are merged
 * in-memory in the service layer.
 *
...
public class AlertReadRepository {
   ...
   *
   * <p>Issues one CQL query per daily bucket in {@code dayBuckets} (at most 2 for a 24h range). The
   * clustering range predicate ({@code timestamp >= ? AND timestamp <= ?}) narrows each bucket to
   * only the rows within the requested window.
   *
    ...
    for (String bucket : dayBuckets) {
      try {
        List<AlertEntity> rows =
            cqlOps.query(cql, mapper, streamId, bucket, fromInstant, toInstant);
        results.addAll(rows);
        log.debug(
            "SPEC-18: alerts fetched: stream={} bucket={} count={}", streamId, bucket, rows.size());
      } catch (Exception e) {
        log.error(
            "SPEC-18: failed to query alerts for stream={} bucket={}: {}",
            streamId,
            bucket,
            e.getMessage(),
            e);

 ...
 * <ul>
 *   <li>SPEC-06 R2: {@link StreamMetricSnapshotDTO} from {@code metrics-aggregated}
 *       → {@code snapshotListenerContainerFactory}</li>
 *   <li>SPEC-14 R1: {@link AlertEventDTO} from {@code alerts}
 *       → {@code alertListenerContainerFactory}</li>
 *   <li>SPEC-14 R2: {@link CbStateEventDTO} from {@code cb-events}
 *       → {@code cbListenerContainerFactory}</li>
 * </ul>
 *
 * <p>Concurrency is set to 1 for each factory — topics have at most 6 partitions
 * but a single consumer thread is sufficient for WebSocket push at current scale.
 *
 * <p>Auto-commit is enabled (default) since message loss on restart is acceptable
 * for live-push paths: the next snapshot arrives within 1 s; the next alert fires
 * only if the condition persists past the cooldown window.
 */
@Configuration
public class KafkaConsumerConfig {
    ...
    /**
     * Consumer factory for {@link StreamMetricSnapshotDTO} payloads from the
     * {@code metrics-aggregated} topic.
     */
    @Bean
    ...
    public ConsumerFactory<String, AlertEventDTO> alertConsumerFactory() {
        ...
        return new DefaultKafkaConsumerFactory<>(props,
                new StringDeserializer(),
                new JsonDeserializer<>(AlertEventDTO.class, false));
    }

    /**
     * Container factory for the {@code alerts} listener
     * ({@link com.streamflow.api.websocket.AlertPushConsumer} — SPEC-14 R1).
     */
    @Bean
    ...
    /**
     * Consumer factory for {@link CbStateEventDTO} payloads from the
     * {@code cb-events} topic.
     */
    @Bean
    public ConsumerFactory<String, CbStateEventDTO> cbConsumerFactory() {
        ...
        return new DefaultKafkaConsumerFactory<>(props,
                new StringDeserializer(),
                new JsonDeserializer<>(CbStateEventDTO.class, false));
    }

    /**
     * Container factory for the {@code cb-events} listener
     * ({@link com.streamflow.api.websocket.CircuitBreakerPushConsumer} — SPEC-14 R2).
     */
    @Bean
     ...
     *
     * <p>Type-info headers are ignored ({@code USE_TYPE_INFO_HEADERS=false}) because
     * the processor serializers suppress them ({@code ADD_TYPE_INFO_HEADERS=false}).
     * Each factory's {@link JsonDeserializer} is constructed with the exact target
     * type, so no header-based type resolution is needed.
     */

 ...
 *
 * <p>SPEC-18 R6: exposes a {@link CassandraTemplate} bean backed by the auto-configured {@link
 * CqlSession} for use by read-only repositories ({@code MetricSnapshotReadRepository} and {@code
 * AlertReadRepository}).
 *
 * <p>{@code schema-action=none} — schema is pre-created by {@code infra/cassandra/init.cql}.
 *
 * <p>The {@code @ConditionalOnBean(CqlSession.class)} guard ensures this bean (and all repositories
 * that depend on it) are silently skipped when Cassandra auto-configuration is excluded in tests
 * that do not need Cassandra.
 */
@Slf4j
@Configuration
public class CassandraConfig {
   ...
   *
   * <p>Only created when {@code CassandraAutoConfiguration} is active and has produced a {@link
   * CqlSession} bean.
   *

 ...
 *
 * <ul>
 *   <li>{@code GET /api/v1/streams} — list all active streams
 *   <li>{@code GET /api/v1/streams/{streamId}} — full snapshot for one stream
 * </ul>
 *
 * <p>All other stream endpoints ({@code /history}, {@code /alerts}, {@code /chaos}) are added in
 * SPEC-11, SPEC-13, and SPEC-18.
 */
@RestController
...
public class StreamController {
   ...
   *
   * <p>SPEC-06 R3: Reads {@code SMEMBERS active_streams} from Redis, then fetches the snapshot JSON
   * for each stream ID.
   *
   ...
   *
   * <p>SPEC-06 R4: Returns HTTP 404 if no snapshot key exists in Redis (mapped by {@link
   * com.streamflow.api.exception.GlobalExceptionHandler}).
   *
  ...
  @Operation(
      ...
      description =
          "Returns the most recent metric snapshot from Redis. "

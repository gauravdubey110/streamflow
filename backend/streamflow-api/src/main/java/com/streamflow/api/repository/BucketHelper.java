 ...
 *
 * <p>SPEC-18 R4: the {@code metric_snapshots} table partitions by {@code stream_id} only
 * (no date bucket), so snapshot queries only need the stream ID and the minute_bucket range.
 * The {@code viewer_events} and {@code alerts} tables partition by
 * {@code (stream_id, date_bucket)}, so queries must enumerate every bucket that could
...
public final class BucketHelper {
    ...
    /**
     * Returns the list of {@code yyyy-MM-dd-HH} (hourly) bucket strings that span
     * the closed interval {@code [from, to]}.
     *
     * <p>Example: {@code from = 2024-06-03T13:45Z}, {@code to = 2024-06-03T15:10Z}
     * → {@code ["2024-06-03-13", "2024-06-03-14", "2024-06-03-15"]}.
     *
    ...
    /**
     * Returns the list of {@code yyyy-MM-dd} (daily) bucket strings that span
     * the closed interval {@code [from, to]}.
     *
     * <p>Example: {@code from = 2024-06-03T22:00Z}, {@code to = 2024-06-04T02:00Z}
     * → {@code ["2024-06-03", "2024-06-04"]}.
     *
     * @param from start of range (inclusive), epoch-milliseconds
     * @param to   end of range (inclusive), epoch-milliseconds

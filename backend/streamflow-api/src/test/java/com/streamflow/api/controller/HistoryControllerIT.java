 ...
 *
 * <p>Seeds Cassandra via a raw {@link CqlSession} before the Spring context starts,
 * then verifies the REST endpoints using {@link RestClient}.
 *
 * <p>Tests:
 * <ul>
 *   <li>AC1 — metric history returns seeded snapshots with non-zero viewer counts, ordered ASC.</li>
 *   <li>AC2 — range exceeding 24h returns HTTP 400 with problem-detail JSON.</li>
 *   <li>AC3 — severity filter on alerts excludes non-matching rows.</li>
 *   <li>AC4 — ETag returned and re-fetch with {@code If-None-Match} returns 304.</li>
 * </ul>
 */
@Testcontainers
@SpringBootTest(
...
class HistoryControllerIT {
        ...
        try (CqlSession session = CqlSession.builder()
            ...
            for (int i = 0; i < 30; i++) {
                ...
                session.execute(
                        ...
                        "VALUES ('it-stream', " + bucket.toEpochMilli() + ", " +
                        (50000L + i) + ", 1.5, 40, 99.0, 50.0, 30.0, 15.0, 5.0)");
            }
            ...
            for (int i = 0; i < 5; i++) {
                ...
                session.execute(
                        ...
                        "message, actual_value) VALUES ('it-stream', '" + today + "', " +
                        ts.toEpochMilli() + ", " + UUID.randomUUID() + ", 'CRITICAL', " +
            ...
            for (int i = 0; i < 3; i++) {
                ...
                session.execute(
                        ...
                        "(stream_id, date_bucket, timestamp, alert_id, severity, alert_type, " +
                        "message, actual_value) VALUES ('it-stream', '" + today + "', " +
                        ts.toEpochMilli() + ", " + UUID.randomUUID() + ", 'INFO', " +
    ...
    /**
     * AC1: GET /history?from=...&to=...&granularity=MINUTE returns ~30 entries,
     * all with non-zero viewer counts, ordered ascending by timestamp.
     */
    @Test
    void ac1_historyEndpoint_returnsSeededSnapshots_orderedAscending() {
        ...
        List<?> raw = client.get()
                .uri("/api/v1/streams/it-stream/history?from={from}&to={to}&granularity=MINUTE",
                        from, to)
                .retrieve()
                .body(List.class);
    ...
    void ac2_rangeExceeding24h_returns400() {
        ...
        ResponseEntity<String> response = client.get()
                ...
                .retrieve()
                .onStatus(status -> status.value() == 400, (req, res) -> { /* suppress throw */ })
    ...
    void ac3_severityFilter_returnsCriticalAlertsOnly() {
        ...
        List<?> raw = client.get()
                .uri("/api/v1/streams/it-stream/alerts?from={from}&to={to}&severity=CRITICAL",
                        from, to)
                .retrieve()
                .body(List.class);
    ...
    /**
     * AC4: ETag is returned on the first call; re-fetch with {@code If-None-Match}
     * matching the ETag returns HTTP 304.
     */
    @Test
    void ac4_etag_and304_roundTrip() {
        ...
        ResponseEntity<Void> first = client.get()
                .uri("/api/v1/streams/it-stream/history?from={from}&to={to}", from, to)
                .retrieve()
                .toBodilessEntity();
        ...
        ResponseEntity<Void> second = client.get()
                .uri("/api/v1/streams/it-stream/history?from={from}&to={to}&If-None-Match={etag}",
                        from, to, etag)
                .retrieve()
                .onStatus(status -> status.value() == 304, (req, res) -> { /* ok */ })
                .toBodilessEntity();

 ...
 *
 * <p>Uses Testcontainers for Kafka and Redis. Rather than producing to Kafka (which introduces
 * unavoidable race conditions between producer and consumer partition assignment), the tests inject
 * payloads directly via {@link SimpMessagingTemplate}. This verifies the STOMP broadcast path — the
 * same approach used in {@link com.streamflow.api.StreamApiIT}.
 *
 * <p>Tests:
 *
 * <ul>
 *   <li>AC1/IT1 — STOMP client on alerts topic receives {@link AlertWsMessage} with {@code type =
 *       "ALERT_FIRED"} and correct fields.
 *   <li>AC2/IT2 — STOMP client on circuit-breaker topic receives {@link CbWsMessage} with {@code
 *       type = "CIRCUIT_BREAKER_STATE_CHANGE"}.
 *   <li>AC3 — Both envelopes conform to plan §10 schema (verified via field assertions on the
...
class AlertAndCbWebSocketIT {
  ...
  /**
   * AC1 / IT1: STOMP subscriber on {@code /topic/streams/{streamId}/alerts} receives an {@link
   * AlertWsMessage} with the correct {@code type} field and all fields populated from the original
   * {@link AlertEventDTO}.
   *
   * <p>The test uses {@link SimpMessagingTemplate} directly (bypassing Kafka) to avoid
   * partition-assignment race conditions.
   */
  @Test
  ...
  /**
   * AC2 / IT2: STOMP subscriber on {@code /topic/streams/all/circuit-breaker} receives a {@link
   * CbWsMessage} with the correct {@code type} field and state fields.
   */
  @Test

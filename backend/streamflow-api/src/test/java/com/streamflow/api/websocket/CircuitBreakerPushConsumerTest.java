 ...
 * <p>Verifies that:
 * <ol>
 *   <li>The correct STOMP destination is used
 *       ({@code /topic/streams/{streamId}/circuit-breaker}).</li>
 *   <li>The {@link CbWsMessage} envelope has
 *       {@code type = "CIRCUIT_BREAKER_STATE_CHANGE"}.</li>
 *   <li>All payload fields are correctly mapped from {@link CbStateEventDTO}.</li>
 * </ol>
 */
...
class CircuitBreakerPushConsumerTest {
    ...
    @InjectMocks
    private CircuitBreakerPushConsumer consumer;

    @Test
    void consume_broadcastsToCorrectDestination() {
        long now = System.currentTimeMillis();
        CbStateEventDTO event = new CbStateEventDTO(
                "all",
                "CLOSED",
                "OPEN",
        ...
        verify(messagingTemplate).convertAndSend(
                eq("/topic/streams/all/circuit-breaker"),
